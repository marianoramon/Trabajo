...
Function SeleccionarEscalaYDistribuirA3( _
    ...
    For Each s As Double In escalasDisponibles
        ...
        Try
            ...
            ' Verificar que vistas proyectadas heredan la escala
            Dim escalasCoinciden As Boolean = True
            If vLateral IsNot Nothing AndAlso Math.Abs(vLateral.Scale - s) > 0.001 Then
                escalasCoinciden = False
            End If
            If vSuperior IsNot Nothing AndAlso Math.Abs(vSuperior.Scale - s) > 0.001 Then
                escalasCoinciden = False
            End If
            If vIso IsNot Nothing AndAlso Math.Abs(vIso.Scale - s) > 0.001 Then
                escalasCoinciden = False
            End If

            If Not escalasCoinciden Then Continue For
...
Function CabeDistribucionA3( _
    ...
    ByVal vIso As DrawingView) As Boolean
        ...
        Try
        Dim margenIzq As Double = 1.2
        Dim margenDer As Double = 1.2
        Dim margenSup As Double = 0.9
        Dim margenSobreCajetin As Double = 0.9

        Dim separacionH As Double = 0.9
        Dim separacionV As Double = 0.6
        Dim reservaCotaX As Double = 0.8
        Dim reservaCotaY As Double = 1.0
        ...
        Dim anchoFila1 As Double = _
            reservaCotaX + vDesarrollo.Width + separacionH + anchoSimbolo + 0.3
        Dim altoFila1 As Double = _
            Math.Max(vDesarrollo.Height + reservaCotaY, altoSimbolo) + 0.2

        ' Fila 2: principal y lateral proyectada.
        Dim anchoFila2 As Double = _
            reservaCotaX + vPrincipal.Width + separacionH + _
            vLateral.Width + reservaCotaX + 0.3
        Dim altoFila2 As Double = _
            Math.Max(vPrincipal.Height + reservaCotaY, _
                     vLateral.Height + reservaCotaY) + 0.2

        ' Fila 3: superior proyectada. Usa todo el ancho disponible.
        Dim anchoFila3 As Double = reservaCotaX + vSuperior.Width + 0.2
        Dim altoFila3 As Double = vSuperior.Height + reservaCotaY + 0.2

        ' Fila 4: isométrica. No se obliga a compartir fila con otra vista larga.
        Dim anchoFila4 As Double = vIso.Width + 0.5
        Dim altoFila4 As Double = vIso.Height + 0.5
